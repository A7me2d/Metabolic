...
@Component({
  ...
  template: `
    <div class="w-full">
      <div class="glass-panel overflow-hidden rounded-[2rem] p-4 md:p-5">
        <div class="relative h-[420px] w-full overflow-hidden rounded-[1.5rem] bg-black">
          <video
            #videoEl
          ...
          <div *ngIf="!streamActive() && !capturedImage()" class="absolute inset-0 flex items-center justify-center">
            ...
            <div *ngIf="error()" class="max-w-sm px-6 text-center">
          ...
          <div *ngIf="streamActive() && !capturedImage()" class="pointer-events-none absolute inset-0">
            <div class="absolute inset-6 rounded-[1.75rem] border border-white/25"></div>
            <div class="absolute left-1/2 top-1/2 w-56 -translate-x-1/2 -translate-y-1/2 text-center text-xs uppercase tracking-[0.3em] text-white/50">
        ...
        <div class="mt-5 flex flex-wrap items-center justify-center gap-3">
          <button
            *ngIf="streamActive() && !capturedImage()"
            (click)="capture()"
            class="flex h-18 w-18 items-center justify-center rounded-full border border-white/20 bg-white p-1 shadow-[0_24px_70px_rgba(255,255,255,0.15)] transition hover:scale-105 active:scale-95">
          ...
          <button
            *ngIf="capturedImage()"
            (click)="retake()"
            class="rounded-2xl border border-white/10 bg-white/5 px-6 py-3 font-semibold text-white transition hover:bg-white/10">
            Retake
          </button>
          ...
          <button
            *ngIf="capturedImage()"
            ...
            class="rounded-2xl bg-linear-to-r from-emerald-400 via-lime-300 to-orange-400 px-6 py-3 font-semibold text-slate-950 transition hover:scale-[1.01]">
            Analyze food
          </button>
          ...
          <button
            *ngIf="!streamActive() && !capturedImage() && !isLoading()"
            (click)="startCamera()"
            class="rounded-2xl bg-linear-to-r from-sky-400 to-cyan-300 px-6 py-3 font-semibold text-slate-950 transition hover:scale-[1.01]">
            Start camera
          </button>
        </div>
      </div>

      <div *ngIf="showConfirmDialog()" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-xl" (click)="showConfirmDialog.set(false)">
        <div class="glass-panel max-w-sm rounded-[2rem] p-6 text-center" (click)="$event.stopPropagation()">
          ...
          <h3 class="text-2xl font-bold text-white">Analyze with AI?</h3>
