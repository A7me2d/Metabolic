      ...
      <div class="mx-auto max-w-6xl space-y-5">
        <header class="glass-panel overflow-hidden rounded-[2rem] p-6 md:p-8">
          <div class="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
            ...
            <div class="rounded-[2rem] border border-white/10 bg-slate-950/35 p-5">
              <div class="flex items-start justify-between gap-4">
                <div class="flex items-center gap-4">
                  <div class="flex h-16 w-16 items-center justify-center rounded-[1.5rem] bg-linear-to-br from-sky-400/25 to-emerald-400/20 text-white">
                  ...
                  <div>
                    ...
                    <div class="mt-1 text-2xl font-bold text-white">{{ user()?.name }}</div>
                    <div class="mt-1 text-sm text-slate-400">{{ user()?.email || 'Signed in user' }}</div>
                  </div>
                </div>

                <button
                  (click)="logout()"
                  class="rounded-2xl border border-rose-300/20 bg-rose-400/10 px-4 py-2 text-sm font-semibold text-rose-100 transition hover:bg-rose-400/15">
                  Logout
                </button>
              </div>
